...
public class ZoommaController {
	...
	public String message(@RequestBody JSONObject resObj) {
		...
		//User Control 부분
		String userKey;
		userKey = (String) resObj.get("user_key");
				
		int userState = userControl.getUserState(userKey);
		int requireCount = userControl.getRequireCount(userKey);
		...
		if (content.contains("안녕")) {
			jobjText.put("text", "안녕 하세요혁기혁기 내부 userKey " + userKey + "requireCount " + requireCount);
